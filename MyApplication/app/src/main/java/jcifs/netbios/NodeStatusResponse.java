...
class NodeStatusResponse extends NameServicePacket {
    ...
    int readRDataWireFormat(byte[] src, int srcIndex) {
        int start = srcIndex;
        this.numberOfNames = src[srcIndex] & Type.ANY;
    ...
    private int readNodeNameArray(byte[] src, int srcIndex) {
        ...
        while (i < this.numberOfNames) {
            try {
                ...
                int hexCode = src[srcIndex + 15] & Type.ANY;
                boolean groupName = (src[srcIndex + 16] & Flags.FLAG8) == Flags.FLAG8;
    ...
    public String toString() {
        return new String(new StringBuffer().append("NodeStatusResponse[").append(super.toString()).append("]").toString());
    }
}
