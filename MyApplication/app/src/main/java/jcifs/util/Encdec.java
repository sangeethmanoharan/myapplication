...
public class Encdec {
    ...
    public static int enc_uint16be(short s, byte[] dst, int di) {
        ...
        dst[di] = (byte) ((s >> TIME_1970_MILLIS_64LE) & Type.ANY);
        dst[di2] = (byte) (s & Type.ANY);
        return TIME_1970_SEC_32LE;
    }

    public static int enc_uint32be(int i, byte[] dst, int di) {
        int i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) ((i >> 24) & Type.ANY);
        di = i2 + TIME_1970_SEC_32BE;
        dst[i2] = (byte) ((i >> 16) & Type.ANY);
        i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) ((i >> TIME_1970_MILLIS_64LE) & Type.ANY);
        dst[i2] = (byte) (i & Type.ANY);
        return TIME_1904_SEC_32LE;
    }

    public static int enc_uint16le(short s, byte[] dst, int di) {
        int di2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) (s & Type.ANY);
        dst[di2] = (byte) ((s >> TIME_1970_MILLIS_64LE) & Type.ANY);
        return TIME_1970_SEC_32LE;
    }

    public static int enc_uint32le(int i, byte[] dst, int di) {
        int i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) (i & Type.ANY);
        di = i2 + TIME_1970_SEC_32BE;
        dst[i2] = (byte) ((i >> TIME_1970_MILLIS_64LE) & Type.ANY);
        i2 = di + TIME_1970_SEC_32BE;
        dst[di] = (byte) ((i >> 16) & Type.ANY);
        dst[i2] = (byte) ((i >> 24) & Type.ANY);
        return TIME_1904_SEC_32LE;
    }

    public static short dec_uint16be(byte[] src, int si) {
        return (short) (((src[si] & Type.ANY) << TIME_1970_MILLIS_64LE) | (src[si + TIME_1970_SEC_32BE] & Type.ANY));
    }

    public static int dec_uint32be(byte[] src, int si) {
        return ((((src[si] & Type.ANY) << 24) | ((src[si + TIME_1970_SEC_32BE] & Type.ANY) << 16)) | ((src[si + TIME_1970_SEC_32LE] & Type.ANY) << TIME_1970_MILLIS_64LE)) | (src[si + TIME_1904_SEC_32BE] & Type.ANY);
    }

    public static short dec_uint16le(byte[] src, int si) {
        return (short) ((src[si] & Type.ANY) | ((src[si + TIME_1970_SEC_32BE] & Type.ANY) << TIME_1970_MILLIS_64LE));
    }

    public static int dec_uint32le(byte[] src, int si) {
        return (((src[si] & Type.ANY) | ((src[si + TIME_1970_SEC_32BE] & Type.ANY) << TIME_1970_MILLIS_64LE)) | ((src[si + TIME_1970_SEC_32LE] & Type.ANY) << 16)) | ((src[si + TIME_1904_SEC_32BE] & Type.ANY) << 24);
    }
    ...
    public static int enc_utf8(String str, byte[] dst, int di, int dlim) throws IOException {
        ...
        while (i2 < dlim && i < strlen) {
            int ch = str.charAt(i);
            if (ch < TIME_1970_SEC_32BE || ch > Service.LOCUS_CON) {
                if (ch <= 2047) {
                    ...
                    i2 = di + TIME_1970_SEC_32BE;
                    dst[di] = (byte) (((ch >> 0) & 63) | Flags.FLAG8);
                    di = i2;
                    ...
                    i2 = di + TIME_1970_SEC_32BE;
                    dst[di] = (byte) (((ch >> TIME_1601_NANOS_64BE) & 63) | Flags.FLAG8);
                    di = i2 + TIME_1970_SEC_32BE;
                    dst[i2] = (byte) (((ch >> 0) & 63) | Flags.FLAG8);
                }
            } else {
                di = i2 + TIME_1970_SEC_32BE;
                dst[i2] = (byte) ch;
    ...
    public static String dec_utf8(byte[] src, int si, int slim) throws IOException {
        ...
        while (si2 < slim) {
            ...
            if (ch < Flags.FLAG8) {
                ...
                ch = src[si2] & Type.ANY;
                uni[ui] = (char) (uni[ui] | (ch & 63));
                if ((ch & 192) != Flags.FLAG8 || uni[ui] < '\u0800') {
                    throw new IOException("Invalid UTF-8 sequence");
                }
            } else {
