...
abstract class NameServicePacket {
    ...
    int nameTrnId;
    int opCode;
    int questionClass = IN;
    int questionCount = IN;
    Name questionName;
    int questionType;
    int rDataLength;
    boolean received;
    ...
    static void writeInt2(int val, byte[] dst, int dstIndex) {
        ...
        dst[dstIndex] = (byte) ((val >> AUTHORITY_OFFSET) & Type.ANY);
    ...
    static int readInt2(byte[] src, int srcIndex) {
        return ((src[srcIndex] & Type.ANY) << AUTHORITY_OFFSET) + (src[srcIndex + IN] & Type.ANY);
    }

    static int readInt4(byte[] src, int srcIndex) {
        return ((((src[srcIndex] & Type.ANY) << 24) + ((src[srcIndex + IN] & Type.ANY) << 16)) + ((src[srcIndex + SRV_ERR] & Type.ANY) << AUTHORITY_OFFSET)) + (src[srcIndex + 3] & Type.ANY);
    }
    ...
    int writeHeaderWireFormat(byte[] dst, int dstIndex) {
        ...
        dst[dstIndex + SRV_ERR] = (byte) ((this.isRecurDesired ? IN : QUERY) + (((((this.opCode << 3) & 120) + (this.isResponse ? Flags.FLAG8 : QUERY)) + (this.isAuthAnswer ? QUESTION_OFFSET : QUERY)) + (this.isTruncated ? SRV_ERR : QUERY)));
    ...
    int readHeaderWireFormat(byte[] src, int srcIndex) {
        ...
        this.isResponse = (src[srcIndex + SRV_ERR] & Flags.FLAG8) != 0;
        ...
        if ((src[(srcIndex + SRV_ERR) + IN] & Flags.FLAG8) == 0) {
            z = false;
        } else {
            z = true;
        }
    ...
    public String toString() {
        ...
        switch (this.resultCode) {
            ...
            case WACK /*7*/:
                ...
                break;
            default:
                str2 = new StringBuffer().append("0x").append(Hexdump.toHexString(this.resultCode, (int) IN)).toString();
                break;
        }
        switch (this.questionType) {
            ...
            case NBSTAT /*33*/:
                ...
                break;
            default:
                questionTypeString = new StringBuffer().append("0x").append(Hexdump.toHexString(this.questionType, (int) QUESTION_OFFSET)).toString();
                break;
        }
        switch (this.recordType) {
            ...
            case NBSTAT /*33*/:
                ...
                break;
            default:
                recordTypeString = new StringBuffer().append("0x").append(Hexdump.toHexString(this.recordType, (int) QUESTION_OFFSET)).toString();
                break;
        }
        StringBuffer append = new StringBuffer().append("nameTrnId=").append(this.nameTrnId).append(",isResponse=").append(this.isResponse).append(",opCode=").append(opCodeString).append(",isAuthAnswer=").append(this.isAuthAnswer).append(",isTruncated=").append(this.isTruncated).append(",isRecurAvailable=").append(this.isRecurAvailable).append(",isRecurDesired=").append(this.isRecurDesired).append(",isBroadcast=").append(this.isBroadcast).append(",resultCode=").append(this.resultCode).append(",questionCount=").append(this.questionCount).append(",answerCount=").append(this.answerCount).append(",authorityCount=").append(this.authorityCount).append(",additionalCount=").append(this.additionalCount).append(",questionName=").append(this.questionName).append(",questionType=").append(questionTypeString).append(",questionClass=");
        if (this.questionClass == IN) {
            str = "IN";
        } else {
            str = new StringBuffer().append("0x").append(Hexdump.toHexString(this.questionClass, (int) QUESTION_OFFSET)).toString();
        }
        ...
        if (this.recordClass == IN) {
            ...
            str = new StringBuffer().append("0x").append(Hexdump.toHexString(this.recordClass, (int) QUESTION_OFFSET)).toString();
        }
