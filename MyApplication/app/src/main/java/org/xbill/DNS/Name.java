...
public class Name implements Serializable, Comparable {
    ...
    public Name canonicalize() {
        ...
        for (i = LABEL_NORMAL; i < this.name.length; i++) {
            if (lowercase[this.name[i] & MAXNAME] != this.name[i]) {
                canonical = false;
                break;
            }
        }
        ...
        Name newname = new Name();
        newname.appendSafe(this.name, offset(LABEL_NORMAL), getlabels());
        for (i = LABEL_NORMAL; i < newname.name.length; i++) {
            newname.name[i] = lowercase[newname.name[i] & MAXNAME];
        }
        return newname;
    }
    ...
    public boolean isWild() {
        ...
        if (!(this.name[LABEL_NORMAL] == (byte) 1 && this.name[1] == (byte) 42)) {
            z = LABEL_NORMAL;
        }
        return z;
    }
    ...
    public boolean isAbsolute() {
        if (labels() != 0 && this.name[this.name.length - 1] == (byte) 0) {
            return true;
        }
        return false;
    }
    ...
    public String toString(boolean omitFinalDot) {
        ...
        if (labels == 1 && this.name[offset(LABEL_NORMAL)] == (byte) 0) {
            return ".";
        }
        StringBuffer sb = new StringBuffer();
        int i = LABEL_NORMAL;
