        ...
        synchronized (Lmhosts.class) {
            ...
            try {
                ...
                if (LogStream.level > 1) {
                    log.println(new StringBuffer().append("lmhosts file: ").append(FILENAME).toString());
                ...
                if (line.length() != 0) {
                    if (line.charAt(0) == '#') {
                        if (line.startsWith("#INCLUDE ")) {
                            String url = new StringBuffer().append("smb:").append(line.substring(line.indexOf(92)).replace('\\', '/')).toString();
                            if (alt > 0) {
                                try {
                                    ...
                                    log.println(new StringBuffer().append("lmhosts URL: ").append(url).toString());
                                    ioe.printStackTrace(log);
                                }
                            } else {
                                populate(new InputStreamReader(new SmbFileInputStream(url)));
