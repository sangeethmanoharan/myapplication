...
public abstract class Record implements Serializable, Cloneable, Comparable {
    ...
    public String toString() {
        ...
        if (!rdata.equals(BuildConfig.VERSION_NAME)) {
            sb.append("\t");
            sb.append(rdata);
        }
        return sb.toString();
    ...
    protected static byte[] byteArrayFromString(String s) throws TextParseException {
        ...
        if (hasEscapes) {
            ...
            if (digits > 0 && digits < 3) {
                throw new TextParseException("bad escape");
            } else if (os.toByteArray().length <= Type.ANY) {
                return os.toByteArray();
            } else {
                throw new TextParseException("text string too long");
            }
        } else if (array.length <= Type.ANY) {
            return array;
        } else {
            throw new TextParseException("text string too long");
        }
    ...
    public int compareTo(Object o) {
        ...
        while (i < rdata1.length && i < rdata2.length) {
            n = (rdata1[i] & Type.ANY) - (rdata2[i] & Type.ANY);
            if (n != 0) {
                return n;
            }
            i++;
