...
public class Handler extends URLStreamHandler {
    ...
    private static URLStreamHandler getDefaultStreamHandler(String protocol) throws IOException {
        synchronized (PROTOCOL_HANDLERS) {
            ...
            if (handler == null) {
                ...
                while (tokenizer.hasMoreTokens()) {
                    ...
                    if (!provider.equals("jcifs")) {
                        className = new StringBuffer().append(provider).append(".").append(protocol).append(".Handler").toString();
                        handlerClass = null;
                        try {
            ...
            if (handler == null) {
                for (String append : JVM_VENDOR_DEFAULT_PKGS) {
                    className = new StringBuffer().append(append).append(".").append(protocol).append(".Handler").toString();
                    handlerClass = null;
                    try {
                        handlerClass = Class.forName(className);
                    } catch (Exception e3) {
            ...
            if (handler == null) {
                throw new IOException(new StringBuffer().append("Unable to find default handler for protocol: ").append(protocol).toString());
            }
