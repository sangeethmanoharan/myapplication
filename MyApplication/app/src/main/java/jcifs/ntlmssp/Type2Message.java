...
public class Type2Message extends NtlmMessage {
        ...
        try {
            ...
            if ((458752 & flags) != 0) {
                if (targetName == null || targetName.length() == 0) {
                    ...
                    target = (flags & 1) != 0 ? targetName.getBytes("UnicodeLittleUnmarked") : targetName.toUpperCase().getBytes(NtlmMessage.getOEMEncoding());
                }
            }
    ...
    public static int getDefaultFlags(Type1Message type1) {
        ...
        int flags = KEYRecord.OWNER_HOST | ((type1Flags & 1) != 0 ? 1 : 2);
    ...
    private void parse(byte[] material) throws IOException {
        ...
        if (bytes.length != 0) {
            target = new String(bytes, (flags & 1) != 0 ? "UnicodeLittleUnmarked" : NtlmMessage.getOEMEncoding());
        }
        setTarget(target);
        for (i = 24; i < 32; i++) {
