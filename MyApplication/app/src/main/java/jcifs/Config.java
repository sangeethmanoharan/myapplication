...
public class Config {
    ...
    static {
        ...
        try {
            BuildConfig.VERSION_NAME.getBytes(DEFAULT_OEM_ENCODING);
        } catch (UnsupportedEncodingException e) {
            logStream = log;
            if (LogStream.level >= 2) {
                log.println(new StringBuffer().append("WARNING: The default OEM encoding ").append(DEFAULT_OEM_ENCODING).append(" does not appear to be supported by this JRE. The default encoding will be US-ASCII.").toString());
            }
    ...
    public static void registerSmbURLHandler() {
        ...
        if (ver.startsWith("1.1.") || ver.startsWith("1.2.")) {
            throw new RuntimeException(new StringBuffer().append("jcifs-0.7.0b4+ requires Java 1.3 or above. You are running ").append(ver).toString());
        }
        String pkgs = System.getProperty("java.protocol.handler.pkgs");
        if (pkgs == null) {
            System.setProperty("java.protocol.handler.pkgs", "jcifs");
        } else if (pkgs.indexOf("jcifs") == -1) {
            System.setProperty("java.protocol.handler.pkgs", new StringBuffer().append(pkgs).append("|jcifs").toString());
        }
    }
    ...
    public static InetAddress getLocalHost() {
        ...
        if (addr != null) {
            try {
                ...
                LogStream logStream = log;
                if (LogStream.level > 0) {
                    log.println(new StringBuffer().append("Ignoring jcifs.smb.client.laddr address: ").append(addr).toString());
                    uhe.printStackTrace(log);
                }
            }
        }
