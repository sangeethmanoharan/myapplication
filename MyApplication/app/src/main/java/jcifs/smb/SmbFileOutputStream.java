...
public class SmbFileOutputStream extends OutputStream {
    ...
    public SmbFileOutputStream(String url, int shareAccess) throws SmbException, MalformedURLException, UnknownHostException {
        this(new SmbFile(url, BuildConfig.VERSION_NAME, null, shareAccess), false);
    }
    ...
    SmbFileOutputStream(SmbFile file, boolean append, int openFlags) throws SmbException, MalformedURLException, UnknownHostException {
        ...
        if ((file instanceof SmbNamedPipe) && file.unc.startsWith("\\pipe\\")) {
            ...
            file.send(new TransWaitNamedPipe(new StringBuffer().append("\\pipe").append(file.unc).toString()), new TransWaitNamedPipeResponse());
        }
        file.open(openFlags, this.access | 2, Flags.FLAG8, 0);
        this.openFlags &= -81;
    ...
    void ensureOpen() throws IOException {
        if (!this.file.isOpen()) {
            this.file.open(this.openFlags, this.access | 2, Flags.FLAG8, 0);
            if (this.append) {
    ...
    public void write(byte[] b, int off, int len) throws IOException {
        if (!this.file.isOpen() && (this.file instanceof SmbNamedPipe)) {
            this.file.send(new TransWaitNamedPipe(new StringBuffer().append("\\pipe").append(this.file.unc).toString()), new TransWaitNamedPipeResponse());
        }
    ...
    public void writeDirect(byte[] b, int off, int len, int flags) throws IOException {
        if (len > 0) {
            ...
            if (LogStream.level >= 4) {
                smbFile = this.file;
                SmbFile.log.println(new StringBuffer().append("write: fid=").append(this.file.fid).append(",off=").append(off).append(",len=").append(len).toString());
            }
            do {
                int w;
