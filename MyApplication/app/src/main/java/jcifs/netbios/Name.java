...
public class Name {
    ...
    int readWireFormat(byte[] src, int srcIndex) {
        ...
        int length = 15;
        for (int i = 0; i < 15; i++) {
            tmp[i] = (byte) (((src[((i * 2) + 1) + srcIndex] & Type.ANY) - 65) << 4);
            tmp[i] = (byte) (tmp[i] | ((byte) (((src[((i * 2) + 2) + srcIndex] & Type.ANY) - 65) & 15)));
            if (tmp[i] != (byte) 32) {
                length = i + 1;
            }
        }
        ...
        this.hexCode = ((src[srcIndex + TYPE_OFFSET] & Type.ANY) - 65) << 4;
        this.hexCode |= ((src[(srcIndex + TYPE_OFFSET) + 1] & Type.ANY) - 65) & 15;
