...
public class Update extends Message {
    ...
    public void present(Name name) {
        newPrereq(Record.newRecord(name, Type.ANY, Type.ANY, 0));
    }

    public void present(Name name, int type) {
        newPrereq(Record.newRecord(name, type, Type.ANY, 0));
    }
    ...
    public void absent(Name name) {
        newPrereq(Record.newRecord(name, Type.ANY, Type.MAILA, 0));
    }

    public void absent(Name name, int type) {
        newPrereq(Record.newRecord(name, type, Type.MAILA, 0));
    }
    ...
    public void delete(Name name) {
        newUpdate(Record.newRecord(name, Type.ANY, Type.ANY, 0));
    }

    public void delete(Name name, int type) {
        newUpdate(Record.newRecord(name, type, Type.ANY, 0));
    }

    public void delete(Name name, int type, String record) throws IOException {
        newUpdate(Record.fromString(name, type, (int) Type.MAILA, 0, record, this.origin));
    }

    public void delete(Name name, int type, Tokenizer tokenizer) throws IOException {
        newUpdate(Record.fromString(name, type, (int) Type.MAILA, 0, tokenizer, this.origin));
    }

    public void delete(Record record) {
        newUpdate(record.withDClass(Type.MAILA, 0));
    }
